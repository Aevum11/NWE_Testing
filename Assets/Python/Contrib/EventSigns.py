...
# globals for data - reduced to essentials
gSavedSigns = None
g_bShowSigns = False
...
def initData():
    ...
    print
    "EventSigns.initData() initializing gSavedSigns"
    global gSavedSigns
...
def initOptions():
    ...
    print
    "EventSigns.initOptions() initializing. g_bShowSigns is %s." % g_bShowSigns
    return True
...
def enabledOptionChanged(pIniObject, bNewValue):
    ...
    print
    "EventSigns.enabledOptionsChanged(%s, %s) resetting g_bShowSigns." % (str(pIniObject), str(bNewValue))
    global g_bShowSigns
...
def placeLandmark(pPlot, sEventType, iFood, iProd, iComm, bIsSign, iSignOwner):
    ...
    * iSignOwner is the player number for the visibility of the sign. If it's -1, all players get one.
    ...
    # Build caption using list join instead of repeated concatenation
    caption_parts = []

    # Note the extra spaces added for separation after each yield adjustment
    if iFood != 0:
        caption_parts.append(TRNSLTR.getText("TXT_KEY_SIGN_FORMAT_FOOD", (iFood,)) + " ")
    if iProd != 0:
        caption_parts.append(TRNSLTR.getText("TXT_KEY_SIGN_FORMAT_PROD", (iProd,)) + " ")
    if iComm != 0:
        caption_parts.append(TRNSLTR.getText("TXT_KEY_SIGN_FORMAT_COMM", (iComm,)) + " ")

    sCaptionDesc = TRNSLTR.getText("TXT_KEY_SIGN_" + sEventType, ())
    sCaption = TRNSLTR.getText("TXT_KEY_SIGN_FORMAT_OVERVIEW",
                               ("".join(caption_parts), "", "", sCaptionDesc))

    if not bIsSign:
        engine.addLandmark(pPlot, sCaption)
    elif iSignOwner == -1:
        # add signs for all valid human players who are still alive.
        iMaxPC = GC.getMAX_PC_PLAYERS()
        for iPlayer in xrange(iMaxPC):
            player = GC.getPlayer(iPlayer)
            if player.isAlive() and player.isHuman():
                addSign(pPlot, iPlayer, sCaption)
                del player  # Free reference immediately
    else:
...
class MapSigns:
    """ A collection of PlotSigns, organized by plot. Uses __slots__ to reduce memory overhead. """
    ...
    def getPlotSigns(self, pPlot):
        ...
        return self.plotDict.get(thisKey, None)
    ...
    def __setitem__(self, pPlot, pPlotSigns):
        ...
        self.setPlotSigns(pPlot, pPlotSigns)
    ...
    def hasPlotSigns(self, pPlot):
        ...
        thisKey = self.__getKey(pPlot)
        return thisKey in self.plotDict

    def __str__(self):
        """ String representation of class instance. """
        # Use list join for efficient string building
        parts = ["MapSigns { plotDict = {"]
        for key, pPlotSigns in self.plotDict.iteritems():
            parts.append("\n\t%s: %s, " % (str(key), str(pPlotSigns)))
        parts.append(" } }")
        return "".join(parts)

    def storeSign(self, pPlot, ePlayer, szCaption):
        """ Stores sign data in the appropriate PlotSigns element. """
        thisKey = self.__getKey(pPlot)
        if not thisKey:
    ...
    def displaySign(self, pPlot, ePlayer):
        ...
        thisKey = self.__getKey(pPlot)
        if not self.hasPlotSigns(pPlot):
            return False

        szCaption = self.plotDict[thisKey].getSign(ePlayer)
        if not szCaption:
            print
            "MapSigns.displaySign() could not show sign; no caption found for player %d on plot %s" % (ePlayer,
                                                                                                       str(thisKey))
            return False
        ...
        if ePlayer == -1:
            if pPlot.isInViewport():
                print
                "MapSigns.displaySign() landmark (%s) shown on plot %s" % (szCaption, str(thisKey))
            ...
            if pPlot.isRevealed(eTeam, False):
                if pPlot.isInViewport():
                    print
                    "MapSigns.displaySign() sign (%s) shown for player %d on plot %s" % (szCaption, ePlayer,
                                                                                         str(thisKey))
                    engine.addSign(pPlot.cloneToViewport(), ePlayer, szCaption.encode('latin_1'))
                return True
            else:
                print
                "MapSigns.displaySign() could not show sign; player %d cannot see plot %s" % (ePlayer, str(thisKey))
        return False
    ...
    def hideSign(self, pPlot, ePlayer):
        ...
        Note that this function uses a temporary MapSigns object for current signs.
        """
        if not pPlot:
        ...
        # Create temporary current signs collection
        currentSigns = MapSigns()
        for iSign in xrange(engine.getNumSigns()):
            pSign = engine.getSignByIndex(iSign)
            signPlot = pSign.getPlot()
            signPlot = MAP.plot(signPlot.getX(), signPlot.getY())
            if signPlot:
                signEPlayer = pSign.getPlayerType()
                szSignCaption = pSign.getCaption()
                if not currentSigns.hasPlotSigns(signPlot):
                    currentSigns[signPlot] = PlotSigns(signPlot)
                currentSigns[signPlot].setSign(signEPlayer, szSignCaption)

        # Check and hide if matching
        if self.hasPlotSigns(pPlot):
            szCaption = self.plotDict[thisKey].getSign(ePlayer)
            if currentSigns.hasPlotSigns(pPlot):
                szExistingCaption = currentSigns[pPlot].getSign(ePlayer)
                if szCaption and szCaption == szExistingCaption:
                    if pPlot.isInViewport():
                        print
                        "MapSigns.hideSign() found matching sign (%s) for player %d on plot %s; will remove it" % (
                            szCaption, ePlayer, str(thisKey))
                        if ePlayer == -1:
                            engine.removeLandmark(pPlot.cloneToViewport())
                        else:
                            engine.removeSign(pPlot.cloneToViewport(), ePlayer)
                    del currentSigns  # Free memory immediately
                    return True
                else:
                    print
                    "MapSigns.hideSign() found sign for player %d saying (%s) instead of (%s) on plot %s; will leave alone." % (
                        ePlayer, szExistingCaption, szCaption, str(thisKey))
            else:
                print
                "MapSigns.hideSign() found no sign on plot %s to remove" % str(thisKey)
        else:
            print
            "MapSigns.hideSign() found no saved signs at all for plot %s" % str(thisKey)

        del currentSigns  # Free memory
        return False
    ...
    def processSigns(self, bShow=None):
        ...
        print
        "MapSigns.processSigns() starting. bShow = %s and g_bShowSigns = %s" % (str(bShow), str(g_bShowSigns))

        if bShow == None:
            bShow = g_bShowSigns

        # Process signs using iterator to save memory
        for pSign in self.plotDict.itervalues():
            pPlot = pSign.getPlot()
            if pPlot:
                print
                "MapSigns.processSigns() Found saved sign data for plot %d, %d ..." % (pPlot.getX(), pPlot.getY())
                for ePlayer in pSign.iterPlayers():  # Use iterator method
                    print
                    "MapSigns.processSigns() ... and caption for player %d" % ePlayer
                    if bShow:
...
class PlotSigns:
    """ Sign information for all players for a given plot. Uses __slots__ to reduce memory overhead. """

    __slots__ = ('iX', 'iY', 'signDict')
    ...
    def getPlayers(self):
        ...
        # Return keys directly as a set for memory efficiency
        return set(self.signDict.keys())

    def iterPlayers(self):
        """ Iterator over player IDs for memory efficiency. """
        return self.signDict.iterkeys()

    def getSign(self, ePlayer):
        """ Returns Caption for a given player on this plot. """
        return self.signDict.get(ePlayer, "")

    def setSign(self, ePlayer, szCaption):
        """ Sets Caption for a given player on this plot. """
        maxPlayers = GC.getMAX_PLAYERS()
        if ePlayer == -1 or (0 <= ePlayer < maxPlayers):
            self.signDict[ePlayer] = szCaption
        else:
            BugUtil.warn(
                "EventSigns PlotSigns.setSign() was passed an invalid Player ID %s at Plot (%d,%d)" % (str(ePlayer),
                                                                                                       self.iX,
                                                                                                       self.iY))
    ...
    def removeSign(self, ePlayer):
        ...
        if ePlayer in self.signDict:
            ...
            BugUtil.warn(
                "EventSigns PlotSigns.removeSign() failed to find a caption for Player %d at Plot (%d,%d)" % (ePlayer,
                                                                                                              self.iX,
                                                                                                              self.iY))
...
class EventSignsEventHandler:
    """ Event Handler for this module. Uses __slots__ to reduce memory overhead. """
    ...
    def onLoadGame(self, argsList):
        ...
        if data:
            ...
            gSavedSigns = data
            print
            "EventSigns Data Loaded:\n %s" % gSavedSigns
            del data  # Free reference
        else:
            print
            "EventSigns has no saved data. Initializing new data."
            initData()
    ...
    def onPreSave(self, argsList):
        ...
        # Commented out as per original - keeping for reference
        pass
    ...
    def onPlotRevealed(self, argsList):
        ...
        if g_bShowSigns and gSavedSigns:
            maxPC = GC.getMAX_PC_PLAYERS()
            for ePlayer in xrange(maxPC):
...
def applySaltpeter(argsList):
    """ Process saltpeter event which may affect multiple plots. """
    kTriggeredData = argsList[1]
